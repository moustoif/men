...
@section('title', 'Personnels en vacance | MEN')
    @section('nav')
    @endsection
    ...
    @section('main-content')
        <div class="w-full h-full justify-center p-3 lg:p-5 flex flex-col gap-y-2">
            <div class="w-full h-10 flex items-center justify-around">
                ...
                <h2 class="text-center font-extrabold mb-1">En congés</h2>
            </div>
            <div class="flex-1 overflow-x-auto shadow-md">
                <table class="w-full">
                    ...
                    <tbody>
                        @foreach ($personnelsEnConge as $enConge)
                            <tr>
                                <td class="table-td">{{ $enConge->matricule }}</td>
                                <td class="table-td">{{ $enConge->cin }}</td>
                                <td class="table-td">{{ $enConge->nom }}</td>
                                <td class="table-td">{{ $enConge->prenom }}</td>
                                <td class="table-td">{{ $enConge->email }}</td>
                                <td class="table-td">{{ $enConge->adresse }}</td>
                                <td class="table-td">{{ $enConge->telephone }}</td>
                                <td class="table-td">{{ $enConge->poste }}</td>
                                <td class="table-td">{{ $enConge->abrev_service }}</td>
                                <td class="table-td">{{ $enConge->abvev_direction }}</td>
                            </tr>
                        @endforeach    
                    </tbody>
                </table>
