...
@section('title', 'Personnels abscents | MEN')
    @section('nav')
    @endsection
    ...
    @section('main-content')
        <div class="w-full h-full justify-center p-3 lg:p-5 flex flex-col gap-y-2">
            <div class="w-full h-10 flex items-center justify-around">
                ...
                <h2 class="text-center font-extrabold mb-1">Abscents</h2>
            </div>
            <div class="flex-1 overflow-x-auto shadow-md">
                <table class="w-full">
                    ...
                    <tbody>
                        @foreach ($abs as $abs)
                            <tr>
                                <td class="table-td">{{ $abs->matricule }}</td>
                                <td class="table-td">{{ $abs->cin }}</td>
                                <td class="table-td">{{ $abs->nom }}</td>
                                <td class="table-td">{{ $abs->prenom }}</td>
                                <td class="table-td">{{ $abs->email }}</td>
                                <td class="table-td">{{ $abs->adresse }}</td>
                                <td class="table-td">{{ $abs->telephone }}</td>
                                <td class="table-td">{{ $abs->poste }}</td>
                                <td class="table-td">{{ $abs->abrev_service }}</td>
                                <td class="table-td">{{ $abs->abvev_direction }}</td>
                            </tr>
                        @endforeach    
                    </tbody>
                </table>
