    ...
    @section('main-content')
        <div class="w-full h-full flex flex-col p-5">
            <div class="border-b border-slate-700 h-10 flex justify-around">
                <a href="{{ url('admin/mes-informations') }}" class="inline-block text-indigo-300 hover:text-indigo-500">&lAarr; Revenir</a>
                <h1 class="text-xl font-extrabold text-center hidden lg:block">Mèttre à jour mon profil</h1>
            </div>

            <form action="{{ url('/updateProfile') }}" method="POST" class="flex-1 pt-1 flex flex-col items-center justify-around">
                @csrf
