    ...
    @section('main-content')
        <div class="w-full h-full justify-center p-3 lg:p-5 flex flex-col gap-y-2">
            <div class="w-full h-10 flex items-center justify-around">
                ...
                <h2 class="text-center font-extrabold mb-1">Liste demande</h2>
            </div>
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        @foreach ($conges as $conge)
                            <tr>
                                <td class="table-td">{{ $conge->matricule }}</td>
                                <td class="table-td">{{ $conge->date_demande }}</td>
                                <td class="table-td">{{ $conge->debut }}</td>
                                <td class="table-td">{{ $conge->fin }}</td>
                                <td class="table-td">{{ $conge->type }}</td>
                                <td class="table-td">{{ $conge->motif }}</td>
                                <td class="table-td">{{ $conge->statut }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
